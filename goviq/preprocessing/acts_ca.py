...
class ActCAPreprocessor(Preprocessor):
    ...
    act_title_regex = r'^(\n?[A-Z][a-z].+?)(?=\s?(?:R\.S\.C\.|S\.C\.|Agreements and ConventionsAssented))'
    ...
    def preprocess(self, path: str, cache: bool = True) -> List[dict]:
        docs = self.load(path)
        processed_docs = []
        for doc in tqdm.tqdm(docs):
            ...
            # TODO: Fix this, landetect not playing nicely with multiprocessing.
        ...
        if cache:
            out_path = os.path.join(self.local_cache, 'processed_' + path)
